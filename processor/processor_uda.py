...
def obtain_label(logger, val_loader, model, distance='cosine', threshold=0):
    ...
    for n_iter, (img, vid, _, _, _) in enumerate(tqdm(val_loader)):
        with torch.no_grad():
            img = img.to(device)
            labels = torch.tensor(vid).clone().detach()
...
def compute_knn_idx(logger, model, train_loader1, train_loader2, feat_memory1, feat_memory2, label_memory1, label_memory2, img_num1, img_num2, target_sample_num=2, topk=1, reliable_threshold=0.0):
    ...
    if topk == 1:
        ...
        _, target_knnidx_c = torch.max(-simmat, 0)
    else:
        ...
        _, target_knnidx_c = torch.topk(-simmat,dim=0,k=topk)
        target_knnidx_c = target_knnidx_c[topk - 1, :]
    ...
    return target_label, knnidx, knnidx_topk, target_knnidx, target_knnidx_c
...
def generate_new_dataset(cfg, logger, label_memory2, s_dataset, t_dataset, knnidx, target_knnidx, target_knnidx_c, target_pseudo_label, label_memory1, img_num1, img_num2, with_pseudo_label_filter=True):
    ...
    use_knnidx = target_knnidx if with_pseudo_label_filter else target_knnidx_c
    # combine_target_sample:
    for idx, data in enumerate(tqdm(target_dataset)):
        t_img_path, _, _, _,t_idx = data
        curidx = use_knnidx[t_idx]
...
def do_train_uda(cfg,
    ...
    for epoch in range(1, int(epochs/2)):
        ...
        if (epoch-1) % update_epoch == 0:
            ...
            target_label, knnidx, knnidx_topk, target_knnidx, target_knnidx_c = compute_knn_idx(logger, model, train_loader1, train_loader2, feat_memory1, feat_memory2, label_memory1, label_memory2, img_num1, img_num2, topk=1, reliable_threshold=0.0)
            del train_loader
            train_loader = generate_new_dataset(cfg, logger, label_memory2, s_dataset, t_dataset, knnidx, target_knnidx, target_knnidx_c, target_label, label_memory1, img_num1, img_num2, with_pseudo_label_filter = False)
        model.train()
        for n_iter, (imgs, vid, target_cam, target_view, file_name, idx) in enumerate(train_loader):
            img = imgs[0].to(device)
            t_img = mu2 * imgs[1].to(device) + (1-mu2) * img  # target img
            ...
            with amp.autocast(enabled=True):
                ...
                #label_1 = torch.zeros(self_score1.shape).to(device)
                #label_2 = torch.zeros(score2.shape).to(device)
                #for i in range(self_score1.shape[0]):
                #    label_1[i, target[i]] = mu1
                #    label_1[i, t_pseudo_target[i]] = 1 - mu1
                #for i in range(score2.shape[0]):
                #    label_2[i, target[i]] = mu2
                #    label_2[i, t_pseudo_target[i]] = 1 - mu2
                loss1 = loss_fn(self_score1, self_feat1, target, target_cam)
                loss2 = loss_fn(score2, feat2, t_pseudo_target, target_cam) * (mu2) + loss_fn(score2, feat2, target, target_cam) * (1 - mu2)
                loss = loss2 + loss1
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (epoch-1) % update_epoch == 0:
            ...
            dynamic_top = 1
            print('source and target topk==',dynamic_top)
            target_label, knnidx, knnidx_topk, target_knnidx, target_knnidx_c = compute_knn_idx(logger, model, train_loader1, train_loader2, feat_memory1, feat_memory2, label_memory1, label_memory2, img_num1, img_num2, topk=dynamic_top, reliable_threshold=0.0)
            del train_loader
            train_loader = generate_new_dataset(cfg, logger, label_memory2, s_dataset, t_dataset, knnidx, target_knnidx, target_knnidx_c, target_label, label_memory1, img_num1, img_num2, with_pseudo_label_filter = True)
        model.train()
        ...
        for n_iter, (imgs, vid, target_cam, target_view, file_name, idx) in enumerate(train_loader):
            ...
            target_view = target_view.to(device)
            with amp.autocast(enabled=True):
                def distill_loss(teacher_output, student_out, conf):
                ...
                confidence1, class1 = self_score1.max(1)[0], self_score1.max(1)[1]
                x1, x2 = self_feat1.cpu().detach().numpy(), feat2.cpu().detach().numpy()
                #sil1 = (silhouette_score(x1, class1.cpu().numpy(), metric='euclidean') * -1 + 1) /2
                confidence2, class2 = score2.max(1)[0], score2.max(1)[1]
                #sil2 = (silhouette_score(x2, class2.cpu().numpy(), metric='euclidean') * -1 + 1) /2

                m = torch.nn.Softmax(dim=0)
                self_score1 = m(torch.mul(self_score1, torch.reshape((confidence2.max() - confidence2), (-1, 1))))
                score2 = m(torch.mul(score2, torch.reshape((confidence1.max() - confidence1), (-1, 1))))

                loss1 = loss_fn(self_score1, self_feat1, target, target_cam)
                loss2 = loss_fn(score2, feat2, t_pseudo_target, target_cam)
                loss3 = distill_loss(score_fusion, score2, confidence2)
